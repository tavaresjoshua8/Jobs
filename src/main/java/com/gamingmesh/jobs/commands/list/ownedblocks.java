...
public class ownedblocks implements Cmd {
	...
	public Boolean perform(Jobs plugin, final CommandSender sender, final String[] args) {
		...
		snd.setTargetName(jPlayer.getName());
		...
		for (BlockTypes type : BlockTypes.values()) {
			...
			HashMap<String, blockLoc> records = ownerShip.get().getBlockOwnerShips().get(jPlayer.getUniqueId());
			if (records == null)
				continue;
			...
			for (Entry<String, blockLoc> record : records.entrySet()) {
				...
				rm.addText(Jobs.getLanguage().getMessage("command.ownedblocks.output.list", "[place]", i, "[type]",
