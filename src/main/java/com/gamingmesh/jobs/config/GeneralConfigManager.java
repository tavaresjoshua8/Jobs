...
public class GeneralConfigManager {
	...
	protected boolean economyAsync, isBroadcastingSkillups, isBroadcastingLevelups, payInCreative,
			payExploringWhenFlying, addXpPlayer, hideJobsWithoutPermission, payNearSpawner, modifyChat,
			saveOnDisconnect, MultiServerCompatability;

	public CMISound soundLevelup;
	public CMISound soundTitleChange;

	public String modifyChatPrefix, modifyChatSuffix, modifyChatSeparator, ServerAccountName, ServertaxesAccountName,
			localeString = "";
	...
	public int jobExpiryTime, BlockProtectionDays, FireworkPower, ShootTime, blockOwnershipRange, globalblocktimer,
			globalBlockBreakTimer, CowMilkingTimer, InfoUpdateInterval, JobsTopAmount, PlaceholdersPage,
			ConfirmExpiryTime, SegmentCount, BossBarTimer, AutoJobJoinDelay, DBCleaningJobsLvl, DBCleaningUsersDays,
	...
	public double skipQuestCost, MinimumOveralPaymentLimit, minimumOveralExpLimit, MinimumOveralPointsLimit,
			MonsterDamagePercentage, DynamicPaymentMaxPenalty, DynamicPaymentMaxBonus, TaxesAmount,
			TreeFellerMultiplier, gigaDrillMultiplier, superBreakerMultiplier;
	...
	public boolean ignoreOreGenerators, useBlockProtection, useNewBlockProtection, useNewExploration,
			useBlockProtectionBlockTracker, enableSchedule, PayForRenaming, PayForEnchantingOnAnvil, PayForEachCraft,
			SignsEnabled, SignsColorizeJobName, ShowToplistInScoreboard, useGlobalTimer, useGlobalBreakTimer,
			useSilkTouchProtection, UseCustomNames, PreventSlimeSplit, PreventMagmaCubeSplit, PreventHopperFillUps,
			PreventBrewingStandFillUps, informOnPaymentDisable, BrowseUseNewLook, payExploringWhenGliding = false,
			resetExploringData = false, disablePaymentIfMaxLevelReached, disablePaymentIfRiding,
			boostedItemsInOffHand = false, boostedItemsInMainHand, boostedArmorItems, boostedItemsSlotSpecific,
			multiplyBoostedExtraValues, addPermissionBoost, highestPermissionBoost, payItemDurabilityLoss,
			applyToNegativeIncome, useMinimumOveralPayment, useMinimumOveralPoints, useMinimumOveralExp,
			useBreederFinder, CancelCowMilking, fixAtMaxLevel, TitleChangeChat, TitleChangeActionBar, LevelChangeChat,
			LevelChangeActionBar, SoundLevelupUse, SoundTitleChangeUse, UseServerAccount, EmptyServerAccountChat,
			EmptyServerAccountActionBar, ShowTotalWorkers, ShowPenaltyBonus, useDynamicPayment, JobsGUIOpenOnBrowse,
			JobsGUIShowChatBrowse, JobsGUISwitcheButtons, ShowActionNames, hideItemAttributes,
			DisableJoiningJobThroughGui, FireworkLevelupUse, UseRandom, UsePerPermissionForLeaving, EnableConfirmation,
			jobsInfoOpensBrowse, MonsterDamageUse, MonsterDamageIgnoreBosses, tameablesPayout, useMaxPaymentCurve,
			blockOwnershipTakeOver, blockOwnershipDisabled, hideJobsInfoWithoutPermission, UseTaxes,
			TransferToServerAccount, TakeFromPlayersPayment, AutoJobJoinUse, AllowDelevel, RomanNumbers,
			BossBarEnabled = false, ActionBarEnabled, ExploreCompact, ExploreSaveIntoDatabase = false,
			DBCleaningJobsUse, DBCleaningUsersUse, DisabledWorldsUse, UseAsWhiteListWorldList, MythicMobsEnabled,
			LoggingUse, payForCombiningItems, payForStackedEntities, payForAbove = false, payForEachVTradeItem,
			allowEnchantingBoostedItems, preventShopItemEnchanting, useCustomFishingOnly = false,
			boostPersistenceEnabled = true;
	...
	private void loadGeneralSettings() {
		...
		c.copyDefaults(true);
		c.addComment("locale-language", "Default language.", "Example: en, ru",
		...
		try {
			...
			Jobs.getPluginLogger()
					.warning("Invalid locale \"" + localeString + "\" defaulting to " + locale.getLanguage());
		}
		...
		c.addComment("MultiServerCompatability",
		...
		c.addComment("Optimizations.DBCleaning.Jobs.Level",
		...
		c.addComment("Optimizations.DBCleaning.Users.Days",
		...
		c.addComment("Optimizations.AutoJobJoin.Delay",
				...
				"For player to auto join job add permission node jobs.autojoin.[jobname]", "Op players are ignored");
		AutoJobJoinDelay = c.get("Optimizations.AutoJobJoin.Delay", 15);

		c.addComment("Optimizations.AllowDelevel",
		...
		c.addComment("Optimizations.RomanNumbers",
		...
		c.addComment("Optimizations.DisabledWorlds.Use",
		...
		c.addComment("Optimizations.DisabledWorlds.UseAsWhiteList",
		...
		c.addComment("broadcast.on-level-up.levels",
		...
		c.addComment("DailyQuests.useGUI", "Defines amount of skips player can do on a quest",
				"This allows player to abandon current quest and get new one");
		DailyQuestsUseGUI = c.get("DailyQuests.useGUI", true);
		c.addComment("DailyQuests.Skips", "Defines amount of skips player can do on a quest",
				"This allows player to abandon current quest and get new one");
		DailyQuestsSkips = c.get("DailyQuests.Skips", 1);
		c.addComment("DailyQuests.SkipQuestCost", "The cost of the quest skip (money).",
		...
		c.addComment("JobExpirationTime", "Fire players if their work time has expired at a job.",
		...
		c.addComment("disable-payment-if-max-level-reached",
		...
		c.addComment("hide-jobs-without-permission",
		...
		c.addComment("hide-jobsinfo-without-permission",
		...
		c.addComment("prevent-shop-item-enchanting",
		...
		c.addComment("enable-pay-creative",
		...
		c.addComment("enable-pay-for-exploring-when-flying",
		...
		if (Version.isCurrentEqualOrHigher(Version.v1_9_R1)) {
			c.addComment("enable-pay-for-exploring-when-gliding",
		...
		if (Version.isCurrentEqualOrHigher(Version.v1_9_R1)) {
			c.addComment("enable-boosted-items-in-offhand",
		...
		c.addComment("enable-boosted-items-in-mainhand",
		...
		c.addComment("boosted-items-slot-specific",
		...
		c.addComment("multiply-boosted-extra-values",
		...
		c.addComment("add-permission-boost",
		...
		c.addComment("pay-for-stacked-entities",
		...
		c.addComment("allow-pay-for-durability-loss",
		...
		c.addComment("allow-pay-for-durability-loss.Use",
		...
		c.addComment("allow-pay-for-durability-loss.WhiteListedItems",
		...
		c.addComment("economy-batch-delay",
		...
		c.addComment("economy-async", "Enable async economy calls.",
		...
		c.addComment("Economy.GeneralMulti",
				...
				"Amounts are in percentage, above 0 will increase payments", "Amount belove 0 will decrease payments",
		...
		c.addComment("Economy.MinimumOveralPayment.use", "Determines minimum payment.",
				...
				"The player can 'get' negative amount of money", "This will limit it to particular percentage",
		...
		c.addComment("Economy.MinimumOveralPoints.use", "Determines minimum payment for points.",
				"In example if player uses McMMO treefeller and earns only 20%, but at same time player gets 25% penalty from dynamic payment.",
				"The player can 'get' negative amount of points", "This will limit it to particular percentage",
				"Works only when original payment is above 0");
		...
		c.addComment("Economy.MinimumOveralExp.use", "Determines minimum payment for experience.",
				"In example if player uses McMMO treefeller and earns only 20%, but at same time player gets 25% penalty from dynamic payment.",
				"The player can 'get' negative amount of experience", "This will limit it to particular percentage",
				"Works only when original payment is above 0");
		...
		c.addComment("Economy.DynamicPayment.use",
		...
		c.addComment("Economy.DynamicPayment.equation",
		...
		try {
			DynamicPaymentEquation = new Parser(
					c.get("Economy.DynamicPayment.equation", "totalworkers / totaljobs / jobstotalplayers - 1"));
		...
		c.addComment("Economy.MaxPayment.curve.use",
				"Enabling this feature will mean players will still earn once they reach cap but "
		...
		c.addComment("Economy.UseServerAccount", "Server economy account",
		...
		c.addComment("Economy.Taxes.AccountName",
		...
		c.addComment("Economy.Taxes.Amount",
		...
		c.addComment("Economy.Limit.Money", "Money gain limit",
		...
		c.addComment("Economy.Limit.Money.ResetTime",
		...
		list = new ArrayList<>();
		c.addComment("Economy.Limit.Point", "Point gain limit",
				"With this enabled, players will be limited how much they can make in defined time");
		...
		c.addComment("Economy.Limit.Point.ResetTime",
				"Time in 24 hour format when limit should reset. This will override TimeLimit if defined",
				"Example: '00:00:00' will reset timer at midnight, '04:30:00' will reset at 4:30 in the morning",
				"Set to empty field if you want to use TimeLimit");
		...
		limit = new CurrencyLimit();
		list = new ArrayList<>();
		c.addComment("Economy.Limit.Exp", "Exp gain limit",
		...
		if (c.get("Economy.Limit.Exp.StopWithPoint", false))
			list.add(CurrencyType.POINTS);
		limit.setStopWith(list);
		c.addComment("Economy.Limit.Exp.Limit",
		...
		c.addComment("Economy.Limit.Exp.ResetTime",
				"Time in 24 hour format when limit should reset. This will override TimeLimit if defined",
				"Example: '00:00:00' will reset timer at midnight, '04:30:00' will reset at 4:30 in the morning",
				"Set to empty field if you want to use TimeLimit");
		...
		c.addComment("Economy.Enchant.AllowEnchantingBoostedItems",
		...
		c.addComment("Economy.VTrade.payForEachTradeItem",
		...
		c.addComment("Economy.Crafting.PayForCombiningItems",
				"true - if players tries to combine (repair) 2 same items, like armor or weapon "
						+ "do they get income for it?");
		payForCombiningItems = c.get("Economy.Crafting.PayForCombiningItems", true);

		c.addComment("Economy.MilkingCow.CancelMilking",
		...
		useBlockProtection = c.get("ExploitProtections.General.PlaceAndBreak.Enabled",
				c.getC().getBoolean("ExploitProtections.General.PlaceAndBreakProtection", true));
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.KeepDataFor", "Only applies when old method is used",
		...
		BlockProtectionDays = c.get("ExploitProtections.General.PlaceAndBreak.KeepDataFor",
				c.getC().getInt("ExploitProtections.General.KeepDataFor", 14));
		...
		c.addComment("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Place",
				"All blocks will be protected X seconds after player places it");
		useGlobalTimer = c.get("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Place.Use",
				c.getC().getBoolean("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Use", true));
		...
		globalblocktimer = c.get("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Place.Timer",
				c.getC().getInt("ExploitProtections.General.PlaceAndBreak.GlobalBlockTimer.Timer", 3));
		...
		c.addComment("ExploitProtections.General.MonsterDamage.Use",
		...
		c.addComment("ExploitProtections.General.MonsterDamage.IgnoreBosses",
		...
		c.addComment("ExploitProtections.McMMO.TreeFellerMultiplier",
		...
		c.addComment("ExploitProtections.McMMO.gigaDrillMultiplier",
		...
		c.addComment("ExploitProtections.McMMO.superBreakerMultiplier",
		...
		c.addComment("ExploitProtections.MythicMobs", "MythicMobs plugin support",
		...
		if (Version.isCurrentLower(Version.v1_14_R1)) {
			c.addComment("ExploitProtections.Spawner.PreventSlimeSplit",
			...
			c.addComment("ExploitProtections.Spawner.PreventMagmaCubeSplit",
		...
		c.addComment("ExploitProtections.Smelt.PreventHopperFillUps",
		...
		c.addComment("ExploitProtections.Smelt.PreventMagmaCubeSplit",
				"Prevent payments when hoppers moving items into brewing stands",
		...
		c.addComment("ExploitProtections.informOnPaymentDisable",
		...
		c.addComment("old-job", "Old job save",
		...
		c.addComment("old-job.level-loss-from-max-level", "Percentage to loose when leaving job at max level",
		...
		c.addComment("ChatText.Messages.DefaultState", "States of chat text messages when payment is issued",
				"Valid options: Off, Batched", "This will be used if player disables action bar payment messages");
		ChatTextMessageDefault = MessageToggleState
				.getByName(c.get("ChatText.Messages.DefaultState", MessageToggleState.Off.toString()));

		c.addComment("ActionBars.Enabled", "Enables ActionBar messages");
		ActionBarEnabled = c.get("ActionBars.Enabled", true);

		c.addComment("ActionBars.Messages.DefaultState", "States of action bar messages",
				"Valid options: " + MessageToggleState.toCommaSeparatedString());
		ActionBarsMessageDefault = MessageToggleState
				.getByName(c.get("ActionBars.Messages.DefaultState", MessageToggleState.Rapid.toString()));

		c.addComment("ActionBars.Messages.KeepFor",
		...
		if (Version.isCurrentEqualOrHigher(Version.v1_9_R1)) {
			...
			c.addComment("BossBar.Messages.DefaultState",
					"When this set to true player will see Bossbar messages by default");
			BossBarsMessageDefault = MessageToggleState
					.getByName(c.get("BossBar.Messages.DefaultState", MessageToggleState.Batched.toString()));

			c.addComment("BossBar.SegmentCount", "Defines in how many parts bossbar will be split visually",
		...
		String SoundLevelupSound = c.get("Sounds.LevelUp.sound", Version.isCurrentLower(Version.v1_9_R1) ? "LEVEL_UP"
				: Version.isCurrentEqualOrHigher(Version.v1_21_R3) ? "ENTITY.PLAYER.LEVELUP" : "ENTITY_PLAYER_LEVELUP")
				.toUpperCase();
		...
		String SoundTitleChangeSound = c.get("Sounds.TitleChange.sound", Version.isCurrentLower(Version.v1_9_R1)
				? "LEVEL_UP"
				: Version.isCurrentEqualOrHigher(Version.v1_21_R3) ? "ENTITY.PLAYER.LEVELUP" : "ENTITY_PLAYER_LEVELUP")
				.toUpperCase();
		...
		c.addComment("Fireworks.LevelUp.Random",
		...
		if (!UseRandom) {
			...
			fireworkEffect = FireworkEffect.builder().flicker(useFlicker).trail(useTrail).with(fwType).withColor(colors)
		...
		c.addComment("JobsBrowse.ShowTotalWorkers",
		...
		c.addComment("JobsBrowse.ShowPenaltyBonus",
		...
		c.addComment("JobsGUI.ShowChatBrowse",
		...
		c.addComment("JobsGUI.SwitcheButtons",
		...
		c.addComment("JobsGUI.RowCount",
				"Defines GUI row count. If set to 0 then size will auto adjust to fit in as many icons as possible");

		JobsGUIRows = CMINumber.clamp(c.get("JobsGUI.RowCount", 0), 0, 6);
		if (JobsGUIRows > 0 && JobsGUIRows < 3 && JobsGUIAddEdge)
			JobsGUIRows = 3;

		c.addComment("JobsGUI.InfoButton.Slot", "Only works if you have AddEdge enabled",
		...
		guiJoinButton = item == null || item.getCMIType() == CMIMaterial.NONE
				? CMIMaterial.GREEN_STAINED_GLASS_PANE.newItemStack()
				: item.getItemStack();
		...
		guiLeaveButton = item == null || item.getCMIType() == CMIMaterial.NONE
				? CMIMaterial.RED_STAINED_GLASS_PANE.newItemStack()
				: item.getItemStack();

		c.addComment("JobsGUI.InfoButton.Commands",
		...
		c.addComment("Commands.PageRow.JobsTop.AmountToShow",
		...
		c.addComment("Commands.PageRow.JobsTop.HiddenPlayers",
		...
		c.addComment("Commands.PageRow.Placeholders.AmountToShow",
		...
		c.addComment("Commands.JobsLeave.EnableConfirmation",
		...
		c.addComment("Commands.JobsInfo.open-browse",
		...
		c.addComment("BlockOwnership.Range",
		...
		c.addComment("BlockOwnership.TakeOver",
		...
		c.addComment("BlockOwnership.Disabled",
